...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'Syslog.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'Syslog.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'Syslog.Client.ProtectedToken'
