...
$parentModule = 'Syslog'
if (-not $ModuleName) { $ModuleName = 'Syslog.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Syslog'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Syslog.psd1"
$prereqHash = @{
